...
from ppo_ae_empc import PPO_Ae_eMpc
from ppo_ae_qmpc import PPO_Ae_qMpc
import logging
...
_CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config', 'dppo.json')
with open(_CONFIG_PATH, 'r') as _f:
    _config = json.load(_f)

args = argparse.Namespace(
    policy=_config.get("policy", "PPO_Ae_MPC"),
    ...
    learning_rate=_config.get("learning_rate", 0.0003),
    max_episodes=_config.get("max_episodes", 6001),
    max_timesteps=_config.get("max_timesteps", 450),
    ...
    gae_lambda=_config.get("gae_lambda", 0.95),
    clip_ratio=_config.get("clip_ratio", 0.2),
    ppo_epochs=_config.get("ppo_epochs", 10),
    mini_batch_size=_config.get("mini_batch_size", 64),
    entropy_coef=_config.get("entropy_coef", 0.01),
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        # --- 创建 Policy 实例 ---
        if not run_test_flag:
            writer = SummaryWriter("./my_experiment/ppo_" + date_time_str)
            if train_policy_q or load_policy_q:
                policy_q = PPO_Ae_qMpc(env, writer=writer if train_policy_q else None, **kwargs_q)
                if not train_policy_q: 
                    policy_q.mode = 'test'
            if train_policy_mpc or load_policy_mpc:
                policy_mpc = PPO_Ae_eMpc(env, writer=writer if train_policy_mpc else None, **kwargs_mpc)
            ...
            # 只创建需要加载的 policy 实例
            if load_policy_q:
                policy_q = PPO_Ae_qMpc(env, writer=None, **kwargs_q)
                policy_q.mode = 'test'
            if load_policy_mpc:
                policy_mpc = PPO_Ae_eMpc(env, writer=None, **kwargs_mpc)
                policy_mpc.mode = 'test'
    ...
    try:
        ...
        if run_test_flag and rank == 0 and statistics is not None:
            ...
            stats_filename = f'./log/{hostname}/ppo_statistics_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.json'
        ...
        if rank == 0:
            if not run_test_flag:
                if train_policy_q and policy_q is not None:
                    ...
                    policy_q.save(policy_q.epoch, policy_path + '/q')
                    logger.info('########################## Q权重Policy model saved #################')
                if train_policy_mpc and policy_mpc is not None:
                    policy_mpc.writer.flush()
                    policy_mpc.writer.close()
                    policy_mpc.save(policy_mpc.epoch, policy_path + '/mpc')
