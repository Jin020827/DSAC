...
class RolloutBuffer_eMpc:
    ...
    def add(self, state, yaw, velocity, action, log_prob, value, reward, done):
        self.states.append(np.asarray(state, dtype=np.float32))
        self.yaws.append(np.asarray(yaw, dtype=np.float32).reshape(1,))
...
class ActorCritic_eMpc(nn.Module):
    ...
    def _features(self, obstacle, yaw, velocity):
        return torch.cat((obstacle, yaw, velocity), dim=-1)

    def forward(self, obstacle, yaw, velocity):
        feature = self._features(obstacle, yaw, velocity)
        mean = self.actor(feature)
        value = self.critic(feature)
        log_std = self.log_std.expand_as(mean).clamp(-5.0, 2.0)
        return mean, log_std, value

    def act(self, obstacle, yaw, velocity, deterministic=False):
        mean, log_std, value = self.forward(obstacle, yaw, velocity)
        if deterministic:
            raw_action = mean
        else:
    ...
    def evaluate_actions(self, obstacle, yaw, velocity, action):
        eps = 1e-6
        action = action.clamp(eps, 1.0 - eps)
...
class PPO_Ae_eMpc(object):
    """PPO actor-critic algorithm for event-triggered MPC update control."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.active:
            ...
            self.actor_critic = ActorCritic_eMpc(action_shape, hidden_dim).to(device)
            self.optimizer = torch.optim.Adam(self.actor_critic.parameters(), lr=lr)
            self.rollout = RolloutBuffer_eMpc()
        else:
    ...
    def generate_n_action(self, env, state_list, velocity_rms):
        ...
        yaw_tensor = torch.from_numpy(yaw_array).float().to(device)
        ...
        with torch.no_grad():
            action, log_prob, value = self.actor_critic.act(
                state_tensor, yaw_tensor, velocity_tensor, deterministic=deterministic
            )
    ...
    def step(self, exp_list):
        ...
        for idx, exp in enumerate(exp_list):
            if exp is None:
                continue
            O_z, O_yaw, O_velocity, action, _, _, _, reward, not_done = exp
    ...
    def _prepare_batch(self, velocity_rms):
        ...
        yaws = torch.FloatTensor(np.asarray(self.rollout.yaws)).to(device)
        ...
        return states, yaws, velocities, actions, old_log_probs, returns, advantages

    def samplefromrollout(self, velocity_rms):
        return self._prepare_batch(velocity_rms)

    def update_actor_critic(self, states, yaws, velocities, actions,
                            old_log_probs, returns, advantages, mb_idx):
        new_log_probs, entropy, values = self.actor_critic.evaluate_actions(
            states[mb_idx], yaws[mb_idx], velocities[mb_idx], actions[mb_idx]
        )
    ...
    def update(self, rollout_batch):
        states, yaws, velocities, actions, old_log_probs, returns, advantages = rollout_batch
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, batch_size, self.mini_batch_size):
                ...
                last_losses = self.update_actor_critic(
                    states, yaws, velocities, actions,
                    old_log_probs, returns, advantages, mb_idx
                )
    ...
    def learn(self, velocity_rms):
        ...
        print(f"[PPO-MPC] training rollout={self.rollout.count}, epoch={self.epoch}")
        ...
        if self.writer is not None and last_losses is not None:
            ...
            self.writer.add_scalar("loss/mpc_policy_loss", policy_loss.item(), self.epoch)
            self.writer.add_scalar("loss/mpc_value_loss", value_loss.item(), self.epoch)
            self.writer.add_scalar("loss/mpc_entropy", entropy_loss.item(), self.epoch)
